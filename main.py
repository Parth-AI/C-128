...
def scrape():
     for i in range(0, 441):
          ...
          print(f'{index+1} page done 1')
def scrape_more_data(hyperlink):
     try:
          page = requests.get(hyperlink)
          Soup = BeautifulSoup(page.content, "HTML.Parser")
          for tr_tag in Soup.find_all("tr", atgrs={"class": "fact_row"}):
               ...
               for td_tag in td_tags:
                    try:
                         ...
                         temp_list.append("")
          new_planet_global.append(temp_list)
     except:
          time.sleep(1)
...
final_planet_data = []
for index,data in enumerate(planet_data):
     new_planet_data_element = new_planet_global[index]
